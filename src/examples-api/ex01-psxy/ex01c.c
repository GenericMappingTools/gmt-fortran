...
int main() {
	...
	id=GMT_Register_IO(API,GMT_IS_DATASET,GMT_IS_REFERENCE,GMT_IS_POINT,GMT_IN,NULL,D);
	printf("Register_IO: %d\n",id);

	/* get a filename for GMT_DATASET */
	ierr=GMT_Encode_ID(API,filename,id);
	printf("Encode_ID: %s\n",filename);

	/* call psxy */
	sprintf(args,"%s %s %s",filename,"-JX16c/24c -R0/11/0/110 -Bx2+lx -By20+ly -BWS+tFig. -Sa1c -N -P","->ex01.ps");
	printf("args = %s\n",args);
