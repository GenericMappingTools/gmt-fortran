...
program example_01
   ...
   ! create GMT session
   API = GMT_Create_Session("example_01", 2, 0, 0)
   ...
   ierr = GMT_Call_Module(API, 'pscoast', &
                          GMT_MODULE_CMD, args)
   ...
   write(args, "(200(a,1x))") trim(filename), '-J -C10 -A50+f7p -Gd4i &
                            ...
                            & -T0.1i/0.02i', '->>ex01.ps'
   ierr = GMT_Call_Module(API, 'grdcontour', &
                          GMT_MODULE_CMD, args)
   ...
   ! call ps2raster
   ierr = GMT_Call_Module(API, 'ps2raster', &
                          GMT_MODULE_CMD, 'ex01.ps -Tg')
